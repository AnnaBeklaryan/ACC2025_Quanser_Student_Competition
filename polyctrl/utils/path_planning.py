...
class RoadMap:
    ...
    def generate_path(self, nodeSequence, spacing=0.001, scale_factor=[1.0, 1.0]):
        """
        ...
        for i in range(len(nodeSequence) - 1):
            ...
            pathSegment = self.find_Dstar_path_weight(
                nodeSequence[i],
                nodeSequence[i+1]
            )
        ...
        path_resampled = self.resample_waypoints(path, spacing=spacing)
        path_resampled[0,:] = scale_factor[0]*path_resampled[0,:]
        path_resampled[1,:] = scale_factor[1]*path_resampled[1,:]
    ...
    def find_Dstar_path_weight(self, startNode, goalNode):
        """Find the shortest path between two nodes using the D* algorithm.
        ...
        trafic_lights = [1,2, 7,8, 11, 12, 15, 17, 20, 22, 23, 24]
        crosswalk = [1,2, 7,8, 11, 12, 15, 17, 20, 22, 23, 24] 

        for i, edge in enumerate(self.edges):
            if i in crosswalk:
                edge.length += 10  
            if i in trafic_lights:
                edge.length += 10 
        ...
        rhs = {node: float('inf') for node in self.nodes} 
        cameFrom = {node: None for node in self.nodes}

        rhs[goalNode] = 0
        heapq.heappush(openSet, (rhs[goalNode] + self._heuristic(goalNode, startNode), goalNode))

        def update_vertex(v):
            """Update the priority queue for a given node v."""
            if rhs[v] != float('inf'):
                heapq.heappush(openSet, (rhs[v] + self._heuristic(v, startNode), v))
            else:
                heapq.heappush(openSet, (gScore[v] + self._heuristic(v, startNode), v))
        ...
        while openSet:
            ...
            if gScore[currentNode] > rhs[currentNode]:  
                gScore[currentNode] = rhs[currentNode]

                for edge in currentNode.inEdges:  
                    predNode = edge.fromNode
                    if edge.length is None:
                        edge.length = 0
                    new_rhs = edge.length + gScore[currentNode]  

                    if new_rhs < rhs[predNode]:  
                        rhs[predNode] = new_rhs
                        cameFrom[predNode] = (currentNode, edge) 
                        update_vertex(predNode)

            if currentNode == startNode:
                break

        if gScore[startNode] == float('inf'):
            return None  

        path = []
        node = startNode
        while node != goalNode:
            if node not in cameFrom or cameFrom[node] is None:
                return None 
            nextNode, edge = cameFrom[node]

            path.append(node.pose[:2, 0]) 
            if edge.waypoints is not None:
                for waypoint in edge.waypoints.T:  
                    path.append(waypoint)

            node = nextNode 

        path.append(goalNode.pose[:2, 0])
